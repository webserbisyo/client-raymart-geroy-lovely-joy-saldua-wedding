...
function parseMapsLink(mapsLink: string): MapExtract {
  ...
  try {
    ...
    const coordMatch = mapsLink.match(
      /@(-?\d+\.\d+),(-?\d+\.\d+)(?:,([\d.]+)z)?/,
    );
...
function getVenueEmbedUrl(venue: ClientVenueData): string | null {
  ...
  if (
    venue.mapsLink &&
    (venue.mapsLink.includes("embed") ||
      venue.mapsLink.includes("output=embed"))
  ) {
...
export function VenueSection({ venue, surface }: VenueSectionProps) {
  ...
  const isShuttle =
    venue.arrivalNote?.toLowerCase().includes("shuttle") ||
  ...
  return (
    <section
      id="venue"
      data-tone={surface}
      className="wedding-section relative overflow-x-clip pt-24 pb-28 px-4"
    >
      <div className="max-w-6xl mx-auto relative z-30">
        <SectionHeading label="Location" title={venue.venueName || "Venue"} />
