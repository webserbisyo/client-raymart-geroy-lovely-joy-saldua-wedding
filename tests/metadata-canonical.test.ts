...
test("safePublicCanonicalUrl returns valid production HTTPS canonical URL for custom domains and client rsvp subdomains", () => {
  assert.equal(
  ...
  assert.equal(
    safePublicCanonicalUrl("https://raymart-and-lovely.rsvp.webserbisyo.com/"),
    "https://raymart-and-lovely.rsvp.webserbisyo.com",
  );
});
...
test("safePublicCanonicalUrl returns undefined for localhost, 127.0.0.1, Vercel, platform domains, and LAN IP dev origins", () => {
  ...
  assert.equal(safePublicCanonicalUrl("https://admin.webserbisyo.com"), undefined);
});
