...
test("WEDDING_BROWSER_THEME_COLOR equals approved dark #23382D", () => {
  assert.equal(
    WEDDING_BROWSER_THEME_COLOR,
    "#23382D",
    "WEDDING_BROWSER_THEME_COLOR must be #23382D",
  );
});
...
test("src/app/manifest.ts exports App Router manifest with #23382D theme and background colors", async () => {
  const manifestData = await manifestGenerator();

  assert.equal(
    manifestData.theme_color,
    "#23382D",
    "manifest theme_color must be #23382D",
  );
  assert.equal(
    manifestData.background_color,
    "#23382D",
    "manifest background_color must be #23382D",
  );
  assert.ok(
